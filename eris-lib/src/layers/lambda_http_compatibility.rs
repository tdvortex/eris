use http::Request;
use tower::{Layer, Service, layer::layer_fn, ServiceExt};
...
pub fn lambda_compatibility_layer<S: Service<Request<HyperBody>> + ServiceExt<Request<HyperBody>>>() -> impl Layer<S> {
    layer_fn(|service: S| service.map_request(|lambda_request: Request<LambdaBody>| {
